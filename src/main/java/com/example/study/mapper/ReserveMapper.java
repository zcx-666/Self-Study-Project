...
public interface ReserveMapper {
    ...
    @Select("SELECT * FROM reserve_form WHERE table_id = #{table_id} AND reserve_id != #{reserve_id} AND create_time < #{create_time} and (reserve_status = 2 or reserve_status = 3 or reserve_status = 4) and not(reserve_start >= #{reserve_end} or reserve_end <= #{reserve_start})")
    ...
    @Select("SELECT * FROM table_form WHERE NOT EXISTS (SELECT reserve_form.table_id FROM reserve_form WHERE (reserve_status = 2 or reserve_status = 3 or reserve_status = 4) AND NOT (reserve_start >= #{reserve_end} OR reserve_end <= #{reserve_start}) AND table_form.table_id = reserve_form.table_id)")
